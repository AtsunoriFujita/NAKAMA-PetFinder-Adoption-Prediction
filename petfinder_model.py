...
class TargetEncoder:
    ...
    def __init__(self, cols, smoothing=1, min_samples_leaf=1,
                 noise_level=0, keep_original=False):
        self.cols = cols
    ...
    def encode(self, train, test, target):
        for col in self.cols:
            if self.keep_original:
                train[col + '_te'], test[col + '_te'] = self.encode_column(
                    train[col], test[col], target)
            else:
                train[col], test[col] = self.encode_column(
                    train[col], test[col], target)
        return train, test
    ...
    def encode_column(self, trn_series, tst_series, target):
        ...
        smoothing = 1 / (1 + np.exp(
            -(averages["count"] - self.min_samples_leaf) / self.smoothing))
        ...
        return self.add_noise(ft_trn_series, self.noise_level), \
               self.add_noise(ft_tst_series, self.noise_level)
